...
async def hent_side_async(page_num, page, per_page, retries=5, timeout=10_000):
    """
    Eksisterende DOM-baserte scraper (uten datofilter i URL).
    Beholdes for andre workflows.
    """
...
# ---------------------------------------------------------
# NY: hent_side_async_url – samme logikk, men URL bygges eksternt
# ---------------------------------------------------------
async def hent_side_async_url(page, url, page_num, retries=5, timeout=10_000):
    """
    Variant av hent_side_async der URL (inkl. datofilter, page, pageSize)
    bygges av kallende kode.

    Brukes av full historical scrape for å utnytte kommunens egen
    server-side datofiltering via hash-URL.
    """

    for attempt in range(1, retries + 1):
        try:
            print(f"[INFO] (async-url) Åpner side {page_num} (forsøk {attempt}/{retries}): {url}")
            ...
            if not loaded:
                if await _is_truly_empty_page(page, page_num):
                    print(f"[INFO] (async-url) Side {page_num} er tom, men gyldig. Returnerer tom liste.")
                    return []
                raise RuntimeError("Innhold ikke lastet / ingen artikler tilgjengelig")

            artikler = await page.query_selector_all("article.bc-content-teaser--item")
            antall = len(artikler)
            print(f"[INFO] (async-url) Fant {antall} dokumenter på side {page_num}")

            if antall == 0:
                if await _is_truly_empty_page(page, page_num):
                    print(f"[INFO] (async-url) Side {page_num} er tom, men gyldig (etter artikler-sjekk).")
                    return []
                raise RuntimeError("0 artikler funnet uten klar indikasjon på tom side")
            ...
            for art in artikler:
                ...
                if detalj_link:
                    try:
                        ...
                        print(f"[WARN] (async-url) Klarte ikke hente filer for {dokid}: {e}")
            ...
            print(f"[WARN] (async-url) Feil ved lasting/parsing av side {page_num} (forsøk {attempt}/{retries}): {e}")

            base_delay = 1.0 * (2 ** (attempt - 1))
            jitter = random.uniform(0, 0.5 * base_delay)
            delay = min(base_delay + jitter, 20.0)

            print(f"[INFO] (async-url) Venter {delay:.2f}s før nytt forsøk på side {page_num}…")
            await asyncio.sleep(delay)

    print(f"[ERROR] (async-url) Side {page_num} feilet etter {retries} forsøk.")
    return None
